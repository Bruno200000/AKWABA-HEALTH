  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Hospitalisation</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium">Gérez l&apos;occupation des lits et le suivi patient en temps réel.</p>
        </div>
        <div className="flex gap-3">
          <div className="bg-slate-100 dark:bg-slate-900 p-1 rounded-xl flex gap-1">
             <button 
               onClick={() => setViewMode("grid")}
               className={cn("p-2 rounded-lg transition-all", viewMode === "grid" ? "bg-white dark:bg-slate-800 shadow-sm text-blue-600" : "text-slate-400")}
             >
                <LayoutGrid className="w-4 h-4" />
             </button>
             <button 
               onClick={() => setViewMode("stats")}
               className={cn("p-2 rounded-lg transition-all", viewMode === "stats" ? "bg-white dark:bg-slate-800 shadow-sm text-blue-600" : "text-slate-400")}
             >
          ...
          <motion.div
            ...
            className={cn(
              ...
              room.status === 'OCCUPIED' 
                ...
                : "bg-slate-50 dark:bg-slate-900/40 border-dashed border-slate-200 dark:border-slate-800 hover:border-blue-300 transition-all"
            )}
          >
            <div className="flex justify-between items-start mb-8 relative z-10">
              <div className={cn(
                "w-12 h-12 rounded-[20px] flex items-center justify-center font-black",
                room.status === 'OCCUPIED' ? "bg-blue-600 text-white shadow-xl shadow-blue-200 dark:shadow-none" : "bg-white dark:bg-slate-800 text-slate-300 border border-slate-100 dark:border-slate-700"
              )}>
                <Bed className="w-6 h-6" />
              </div>
              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-3 py-1 bg-white/50 dark:bg-slate-800 rounded-full border border-slate-100 dark:border-slate-700">{room.type}</span>
            </div>

            <div className="relative z-10">
              <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Chambre {room.room_number}</h3>
              {room.status === 'OCCUPIED' ? (
                <div className="mt-6 space-y-4">
                   ...
                   <button className="w-full mt-6 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-500 hover:bg-red-50 hover:text-red-600 hover:border-red-100 transition-all flex items-center justify-center gap-2">
            ...
            {room.status === 'OCCUPIED' && (
               <div className="absolute -top-12 -right-12 w-32 h-32 bg-blue-50/50 dark:bg-blue-900/10 rounded-full blur-3xl" />
            )}
          </motion.div>
        )) : (
