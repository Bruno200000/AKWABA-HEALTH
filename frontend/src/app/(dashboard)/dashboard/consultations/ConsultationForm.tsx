      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
            <div className="relative">
              ...
              {searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl z-20 overflow-hidden">
                  {searchResults.map((m) => (
                    <button
                      key={m.id}
                      type="button"
            ...
            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {formData.prescription.map((item) => (
                <div key={item.id} className="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm space-y-3 relative group">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      ...
                      <input 
                        ...
                        className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl text-xs outline-none focus:ring-2 focus:ring-blue-500/10" 
        ...
        {activeStep === 4 && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-8">
            <div className="bg-slate-50 dark:bg-slate-900/50 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 space-y-6">
               <div className="flex justify-between items-start">
                  <div>
                  ...
                  <div className="w-16 h-16 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-2 flex items-center justify-center">
               ...
               <div className="space-y-4">
                  <div className="flex justify-between text-xs">
                     ...
                     <span className="font-black text-slate-900 dark:text-white truncate max-w-[200px]">{formData.diagnosis}</span>
                  </div>
                  <div className="flex justify-between text-xs">
                     <span className="font-bold text-slate-400 uppercase">Ordonnance</span>
                     <span className="font-black text-slate-900 dark:text-white">{formData.prescription.length} Médicament(s)</span>
                  </div>
               </div>
               ...
               <div className="pt-6 border-t border-slate-200 dark:border-slate-800">
                  ...
                  <div className="h-40 bg-white dark:bg-slate-800 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-700 relative group cursor-crosshair">
            ...
            <div className="flex gap-4">
              ...
              <button disabled={isLoading} type="submit" className="flex-[2] py-4 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-blue-200 dark:shadow-none hover:bg-blue-700 transition-all flex items-center justify-center gap-2">
