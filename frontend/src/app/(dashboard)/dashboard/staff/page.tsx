  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Gestion du Personnel</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium">Gérez votre équipe médicale et suivez leurs performances.</p>
        </div>
        <button 
            ...
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex gap-2">
                {["Tous", "Médecins", "Infirmiers"].map((tab) => (
                  <button key={tab} className="px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold hover:bg-slate-50 transition-all first:bg-blue-600 first:text-white first:border-blue-600">
                    {tab}
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {isLoading ? (
                ...
                <motion.div
                  ...
                  className="bg-white dark:bg-slate-900 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group overflow-hidden"
                >
                  <div className="p-8">
                    <div className="flex justify-between items-start mb-6">
                      <div className="relative">
                        <div className="w-16 h-16 rounded-[20px] bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center font-black text-xl text-blue-600 shadow-inner uppercase">
                          {staff.first_name?.[0]}{staff.last_name?.[0]}
                        </div>
                        <div className={cn(
                          "absolute -bottom-1 -right-1 w-4 h-4 border-2 border-white dark:border-slate-900 rounded-full shadow-sm",
