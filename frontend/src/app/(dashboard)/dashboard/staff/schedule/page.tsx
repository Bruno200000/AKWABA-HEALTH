  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Planning du Personnel</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium">Gérez les rotations et les horaires de garde de l'équipe.</p>
        </div>
      ...
      {/* Week Selector */}
      <div className="flex bg-white dark:bg-slate-900 p-2 rounded-[24px] border border-slate-200 dark:border-slate-800 shadow-sm overflow-x-auto custom-scrollbar">
        {days.map((day, i) => (
          <button
            key={day}
            ...
            className={cn(
              ...
              activeDay === i ? "bg-slate-900 text-white dark:bg-white dark:text-slate-900" : "text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800"
            )}
          >
            {day}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {schedule.map((staff, i) => (
          <motion.div
            ...
            className="p-6 bg-white dark:bg-slate-900 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between group"
          >
            <div className="flex items-center gap-6">
               <div className="w-16 h-16 rounded-[24px] bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex items-center justify-center relative">
