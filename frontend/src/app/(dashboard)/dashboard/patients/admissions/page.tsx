...
export default function AdmissionsPage() {
  ...
  return (
    <div className="space-y-8 pb-20">
      <div>
        <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Admissions en cours</h1>
        <p className="text-slate-500 dark:text-slate-400 font-medium">Suivez les patients en cours d'admission dans l'établissement.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {admissions.map((adm) => (
          <motion.div
            key={adm.id}
            whileHover={{ y: -5 }}
            className="p-6 bg-white dark:bg-slate-900 rounded-[32px] border border-slate-200 dark:border-slate-800 shadow-sm"
          >
            ...
            <div className="flex gap-2">
               <button className="flex-1 py-3 bg-slate-50 dark:bg-slate-800 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-400 hover:bg-blue-600 hover:text-white transition-all">
                  Détails
               </button>
