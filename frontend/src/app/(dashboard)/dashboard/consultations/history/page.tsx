  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Historique des Consultations</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium">Archive complète de toutes les consultations passées.</p>
        </div>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {history.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50/50 transition-colors cursor-pointer group">
                ...
                <td className="px-8 py-6">
                   <span className="text-sm text-slate-600 dark:text-slate-400 italic">"{item.diagnosis}"</span>
                </td>
                <td className="px-8 py-6">
